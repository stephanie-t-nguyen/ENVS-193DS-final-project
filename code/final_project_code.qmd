...
```{r}
#Exploratory data visualization, quite hard to tell the relationship between location and number of seeds, there are a lot of data clustered at zero
ggplot() + 
  geom_jitter(data = kangaroo_rat, aes(x = loc, y = seeds, color = loc), alpha = 0.4) + 
  theme_bw() + 
...
This figure plots observations of the number of seeds found at each mound location.
...
```{r}
kruskal.test(seeds ~ loc, data = kangaroo_rat) #p-value very small, evidence to reject the null, evidence that there is a difference in seed numbers among the mound locations
```
